Module.register("MMM-AroundNow", {
	...
	getDom () {
		...
		header.className = "aroundnow-header";
		for (let d = -this.config.hoursBefore / this.config.hoursBefore; d <= 0; d++) { /*noop*/ }
		...
		const nowPosition = (this.config.hoursBefore * this.config.pixelsPerHour);
